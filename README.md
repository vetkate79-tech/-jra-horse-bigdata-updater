...
競馬ワードは、JRA公式「競馬用語辞典」と「海外競馬英和辞典」を最優先ソースとして定期取得します。カテゴリーは国内辞典20分類・海外英和辞典10分類の正式名称と順序をそのまま使用し、複数カテゴリー所属も保持します。サイト固有語も独自カテゴリーを増やさず、JRA分類へ対応づけます。用語・読み・分類・出典URLを保持し、JRAの説明文は大量転載せず、このサイト独自の短い案内文と公式出典リンクを表示します。トップページは自動用語リンクの対象外です。
