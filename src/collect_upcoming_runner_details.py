...
OUT=Path('docs/data/horses/weekly_runner_details.json')
STATUS=Path('status/weekly_runner_details.json')
...
def parse_card(cname,raw):
    ...
    for a in soup.find_all('a'):
        ...
        tr=a.find_parent('tr'); cells=[]; row_text=''
        if tr:
...
def main():
    ...
    uniq={c['race_id']:c for c in cards};cards=sorted(uniq.values(),key=lambda x:(x['date'],x['track'],x['race_no']))
    runners=[];missing=[]
    for race in cards:
        for row in race['runners']:
            h=by_id.get(row['horse_id'])
            if h is None:
                h={'horse_id':row['horse_id'],'horse_name':row['horse_name']};missing.append(row['horse_id'])
            runners.append(runner_detail(race,row,h))
    dates=sorted({r['race']['date'] for r in runners})
    payload={'summary':{
        'status':'READY' if runners else 'NO_UPCOMING_RACECARDS','race_count':len(cards),'runner_count':len(runners),
        'dates':dates,'missing_master_count':len(set(missing)),
        'policy':'heavy detail exists only for verified upcoming JRA runners'
    },'runners':runners}
    OUT.parent.mkdir(parents=True,exist_ok=True);STATUS.parent.mkdir(parents=True,exist_ok=True)
    OUT.write_text(json.dumps(payload,ensure_ascii=False,separators=(',',':')),encoding='utf-8')
    STATUS.write_text(json.dumps({**payload['summary'],'errors':errors,'missing_master_ids':sorted(set(missing))},ensure_ascii=False,indent=2),encoding='utf-8')
