...
for r in rows:
    ...
    base={'race_id':clean(r.get('race_id')),'horse_id':clean(r.get('horse_id')),'date':date,'course':clean(r.get('course') or r.get('場')),
      'race_no':clean(r.get('race_no') or r.get('R')),'race_name':clean(r.get('race_name')),'grade':g,
      'surface':clean(r.get('surface') or r.get('芝ダ')),'distance_m':clean(r.get('distance_m') or r.get('距離')),
...
for name,bucket in selected.items():
    ...
    item={'horse_name':name,'horse_id':hid,'sex_age':clean(p.get('sex_age_latest') or latest.get('sex_age')),
      ...
      'latest_race_date':clean(p.get('latest_race_date') or latest.get('date')),'latest_course':clean(p.get('latest_course') or latest.get('course')),
      'latest_surface':clean(p.get('latest_surface') or latest.get('surface')),'latest_distance_m':clean(p.get('latest_distance_m') or latest.get('distance_m')),
...
for h in horses:
    for s in h['target_starts']:
        by_date[s['date']][s['course']][str(s['race_no'])].append({'horse_name':h['horse_name'],'horse_id':h['horse_id'],'horse_no':s['horse_no'],'finish':s['finish'],'jockey':s['jockey'],'sex_age':s['sex_age']})
...
summary={'generated_from':sources,'horse_count':len(horses),'graded_horse_count':sum(bool(h['graded_starts']) for h in horses),
 'target_weekend_horse_count':sum(bool(h['target_starts']) for h in horses),'target_dates':sorted(TARGET_DATES),
 'target_race_count_by_date':race_counts,'target_runner_rows_by_date':runner_counts,
 'missing_horse_ids':sum(not h['horse_id'] for h in horses),
 'coverage_note':'G1-G3経験はリポジトリに存在するJRA公式結果データの範囲で抽出。8/29・8/30は各日36Rの中央競馬全レースを品質確認して収録。'}
