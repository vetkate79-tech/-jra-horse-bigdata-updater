...
PRED=Path('docs/data/oral-v10-72-connected-durability-sealed.json');RES=Path('data/race_results_html_2026.csv');V9=Path('docs/data/oral-v9-72-confidence-style-scored.json');OUT=Path('docs/data/oral-v10-72-connected-durability-scored.json');STATUS=Path('status/oral-v10-72-connected-durability-scored.json')
...
def main():
 ...
 for r in p['races']:
  ...
  rows.append({'date':k[0],'track':k[1],'race_no':k[2],'decision':a.get('pre_market_decision'),'axis':axis,'axis_finish':fin,'axis_grade':grade,'trio_hit':hit,'durability_status':(a.get('axis_durability') or {}).get('status')})
 ...
 if V9.exists():
  old=json.loads(V9.read_text()).get('summary',{});summary['vs_v9']={'axis_top3_delta':(g['HIT']+g['PLACE'])-(int(old.get('axis_1st',0))+int(old.get('axis_2nd_3rd',0))),'trio_hits_delta':th-int(old.get('trio_hits',0)),'bought_races_delta':tb-int(old.get('trio_bought_races',0))}
