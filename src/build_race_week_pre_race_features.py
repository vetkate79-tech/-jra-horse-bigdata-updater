...
def draw_priors(rows):
    ...
    for r in rows:
        ...
        k=(track,surface,int(round(dist/200.0)*200),frame);buckets[k][0]+=1;buckets[k][1]+=int(finish<=3)
...
def continuity(hist,race):
    ...
    last=hist[0];target_surface=norm_surface(race.get('surface'));last_surface=norm_surface(last.get('surface'));td=integer(race.get('distance_m'));ld=integer(last.get('distance_m'));surface_score=1.0 if target_surface and target_surface==last_surface else .35;distance_score=.5 if not td or not ld else max(0.0,1-abs(td-ld)/800);rd=dt(race.get('date'));ldate=dt(last.get('race_date'));days=(rd-ldate).days if rd and ldate else None
...
def feature_for(runner,hist,rates,field_sizes,draw_rate):
    race=runner.get('race') or {};starts=len(hist);top3=sum(integer(x.get('finish_position'),99)<=3 for x in hist);show=(top3+1.5)/(starts+5) if starts else .30;recent=hist[:5]
    if recent:
    ...
    cond=(sum(integer(x.get('finish_position'),99)<=3 for x in matches)+1)/(len(matches)+3) if matches else show;last3f=[num(x.get('last3f')) for x in recent];last3f=[x for x in last3f if x is not None];l3=.5 if not last3f else max(0,min(1,(40-min(last3f))/8));uncertainty=1-min(1,starts/5)
    j_all,t_all,j30,t30=rates;jockey=runner.get('jockey') or '';trainer=runner.get('trainer') or '';ja=j_all(jockey);ta=t_all(trainer);jr=j30(jockey);tr=t30(trainer);style,style_starts=style_from_history(hist,field_sizes);surf_cont,dist_cont,rest_info=continuity(hist,race);days_since,rest_fit=rest_info if rest_info else (None,.5);frame=integer(runner.get('frame_no'));draw,draw_n=draw_rate(str(race.get('track') or ''),target_surface,target_distance,frame);frame_known=frame is not None
    structural=100*(.65*show+.25*cond+.05*ja+.05*ta);medium=100*(.70*rec+.30*l3);short=100*(.25*jr+.25*tr+.20*rest_fit+.10*surf_cont+.10*dist_cont+.10*draw);score=STRUCTURAL_WEIGHT*structural+MEDIUM_WEIGHT*medium+SHORT_WEIGHT*short-8*uncertainty
    return {'starts_before':starts,'show_rate_prior':round(show,4),'recent_form':round(rec,4),'condition_fit':round(cond,4),'uncertainty':round(uncertainty,4),'jockey_show_prior':round(ja,4),'trainer_show_prior':round(ta,4),'jockey_show_30d':round(jr,4),'trainer_show_30d':round(tr,4),'last3f_signal':round(l3,4),'days_since_last_start':days_since,'surface_continuity':round(surf_cont,4),'distance_continuity':round(dist_cont,4),'frame_no':str(frame) if frame_known else '','frame_known':frame_known,'draw_show_prior':round(draw,4),'draw_history_starts':draw_n,'pre_race_running_style':style,'running_style_sample_starts':style_starts,'structural_big_data_score':round(structural,3),'medium_term_form_score':round(medium,3),'short_term_context_score':round(short,3),'structural_weight':STRUCTURAL_WEIGHT,'medium_weight':MEDIUM_WEIGHT,'short_term_weight':SHORT_WEIGHT,'short_term_weight_cap':SHORT_WEIGHT_MAX,'pre_race_score':round(score,3),'pre_race_score_source':'JRA_MULTI_HORIZON_STRICT_CUTOFF_V4_CARD_HISTORY_DRAW_SAFE'}

def merge_history(primary,card_rows,cutoff):
    out=[];seen=set()
    for r in list(primary or [])+list(card_rows or []):
        d=str(r.get('race_date') or '')
        if not d or d>=cutoff:continue
        k=(d,str(r.get('course') or r.get('track') or ''),str(r.get('finish_position') or ''),str(r.get('distance_m') or ''),norm_surface(r.get('surface')))
        if k in seen:continue
        seen.add(k);out.append(r)
    out.sort(key=lambda r:str(r.get('race_date') or ''),reverse=True);return out
...
def main():
    ...
    rates=people_rates(rows,cutoff);draw_rate=draw_priors(rows);feats=[];id_joins=name_joins=card_history_horses=0
    for x in runners:
        race=x.get('race') or {};hid=str(x.get('horse_id') or '');name=clean_name(x.get('horse_name'));base=by_horse.get(hid,[]);join_source='HORSE_ID'
        if base:id_joins+=1
        elif name and len({z for z in name_ids.get(name,set()) if z})==1:base=by_name.get(name,[]);join_source='EXACT_HORSE_NAME';name_joins+=int(bool(base))
        else:join_source='NO_HISTORY_MATCH'
        card_rows=x.get('recent_starts') or [];hist=merge_history(base,card_rows,cutoff);card_history_horses+=int(bool(card_rows));
        if not base and card_rows:join_source='JRA_CURRENT_RACECARD_PRIOR_STARTS'
