...
CATEGORY_DISPLAY_LABELS={
    ...
    '競馬史上のもの（現存のものは除く）':'歴史',
    '競馬に関わる人々':'人物',
    '競馬関係の法律、施行規程、公正確保など':'規則',
    '厩舎ことば':'厩舎',
    '馬体の各部位、特徴、個体の分類など':'馬体',
    '馬の病気、ケガ、能力をそこなうものなど':'疾病',
    '資格・成績・ランキング・表彰など':'成績',
    '馬の脚質、あしいろ、歩様、距離特性など':'脚質',
    'JRA主催のイベント、ファンサービスなど':'催事',
    'その他':'その他',
}
LOCAL_CATEGORY_MAP={
...
def write_index(terms):
    ...
    chips=''.join(f'<button class="chip" data-filter="{html.escape(c)}" title="{html.escape(c)}">{html.escape(display_category(c))} {len(groups[c])}</button>' for c in CATEGORY_ORDER)
    page=f'''<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>競馬ワード索引｜JRA AI</title>{base_css()}</head><body><main class="wrap"><header class="top"><a href="../">JRA AI</a><a href="../consult/">AIに相談</a></header><section class="hero"><h1>競馬ワード索引</h1><p>このページだけで意味まで確認できます。分類はJRA公式「競馬用語辞典」に準拠し、表示名は探しやすい一語に簡略化しています。</p></section><input id="q" class="search" type="search" placeholder="ワード・読み方・関連語を検索"><div class="cats"><button class="chip" data-filter="ALL">すべて {len(terms)}</button>{chips}</div><div id="groups">{''.join(cards)}</div></main><script>const q=document.querySelector('#q');let cat='ALL';function n(s){{return String(s||'').normalize('NFKC').toLowerCase().replace(/[\\s・･._\\-ー()（）]/g,'')}}function run(){{const z=n(q.value);document.querySelectorAll('.term').forEach(a=>{{const okCat=cat==='ALL'||a.dataset.cat===cat;const okQ=!z||n(a.textContent).includes(z);a.style.display=okCat&&okQ?'block':'none'}});document.querySelectorAll('.group').forEach(g=>{{const has=[...g.querySelectorAll('.term')].some(a=>a.style.display!=='none');const empty=g.querySelector('.empty');const showEmpty=!!empty&&!z&&(cat==='ALL'||g.dataset.group===cat);g.style.display=has||showEmpty?'block':'none'}})}}q.oninput=run;document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{{cat=b.dataset.filter;run()}})</script></body></html>'''
...
def write_term_pages(terms):
    ...
    if OUT.exists():
        for d in OUT.iterdir():
            if d.is_dir() and d.name not in expected: shutil.rmtree(d)
    for t in terms:
        d=OUT/slug(t['term']);d.mkdir(parents=True,exist_ok=True)
        aliases=t.get('aliases') or []
        source=''
        if t.get('source_url'):
            source=f'<p class="source-note">参考：<a href="{html.escape(t["source_url"])}" target="_blank" rel="noopener">{html.escape(t.get("source_name") or "参照元")}</a></p>'
        alias_html=f'<div class="aliases">関連語：{html.escape("・".join(aliases))}</div>' if aliases else ''
        page=f'''<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{html.escape(t['term'])}とは｜JRA AI 競馬ワード</title>{base_css()}</head><body><main class="wrap"><header class="top"><a href="../../">JRA AI</a><a href="../">ワード索引</a></header><section class="hero"><div class="reading">{html.escape(t.get('reading') or '')}</div><h1>{html.escape(t['term'])}</h1><p>{html.escape(category_label(t))}</p></section><section class="term"><h2 class="meaning-title">意味</h2><p class="summary" data-word-links-scope>{html.escape(t['summary'])}</p>{alias_html}{source}</section><a class="back" href="../">← ワード索引へ戻る</a></main><script src="/-jra-horse-bigdata-updater/word-links.js"></script></body></html>'''
...
def main():
    ...
    expected={slug(t['term']) for t in terms}
    if generated!=expected: raise RuntimeError(f'dictionary page mismatch: expected={len(expected)} generated={len(generated)}')
