...
ROOT=Path('.')
DOCS=ROOT/'docs'
SOURCES=[DOCS/'data/glossary.json',DOCS/'data/site-terms.json',DOCS/'data/site-term-candidates.json']
OUT=DOCS/'words'
INDEX=DOCS/'data/word-index.json'

CATEGORY_ORDER=['馬券','予想','脚質','展開','ペース','能力','成績','クラス','重賞','馬データ','血統・調教','市場','詳細分析','AI・機能','レース前','サイト内候補','競馬用語','その他']
...
def load_terms():
    ...
    for src in SOURCES:
        ...
        for x in doc.get('terms',[]):
            ...
            merged={**old,**x,'aliases':aliases}
            if not merged.get('summary'): merged['summary']=merged.get('short_definition') or 'サイト内で使われている用語です。'
            by[k]=merged
...
def base_css():
    return '''<style>:root{--bg:#f7f8fb;--paper:#fff;--ink:#17202a;--muted:#6d7784;--line:#e2e7ee;--blue:#2563a8;--soft:#eef5ff}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Hiragino Sans","Yu Gothic",sans-serif}.wrap{max-width:900px;margin:auto;padding:18px 15px 80px}.top{display:flex;justify-content:space-between;align-items:center;padding:10px 0}.top a{text-decoration:none;color:var(--ink);font-weight:900}.hero{padding:20px 0}.hero h1{font-size:30px;margin:0 0 8px}.hero p{color:var(--muted);font-size:13px;line-height:1.7}.search{width:100%;padding:14px;border:1px solid var(--line);border-radius:14px;background:var(--paper);font-size:16px}.cats{display:flex;gap:7px;overflow:auto;padding:12px 0}.chip{border:1px solid #cbd9eb;background:#fff;color:var(--blue);border-radius:999px;padding:8px 11px;white-space:nowrap;font-weight:800;font-size:12px}.group{margin-top:20px}.group h2{font-size:17px;margin:0 0 9px}.grid{display:grid;gap:8px}.term{display:block;text-decoration:none;color:var(--ink);background:var(--paper);border:1px solid var(--line);border-radius:14px;padding:13px}.term b{color:var(--blue)}.term small{display:block;color:var(--muted);margin-top:4px}.reading{color:var(--muted);font-size:11px}.summary{font-size:14px;line-height:1.85}.aliases{background:var(--soft);padding:10px 12px;border-radius:12px;color:#345b83;font-size:12px}.back{display:inline-block;color:var(--blue);font-weight:800;text-decoration:none;margin-top:18px}@media(min-width:700px){.grid{grid-template-columns:repeat(2,1fr)}}</style>'''
...
def write_index(terms):
    ...
    page=f'''<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>競馬ワード索引｜JRA AI</title>{base_css()}</head><body><main class="wrap"><header class="top"><a href="../app/">JRA AI</a><a href="../consult/">AIに聞く</a></header><section class="hero"><h1>競馬ワード索引</h1><p>サイトで使っている言葉をカテゴリから探せます。検索でも、一覧からでも確認できます。</p></section><input id="q" class="search" type="search" placeholder="ワード・読み方・関連語を検索"><div class="cats"><button class="chip" data-filter="ALL">すべて {len(terms)}</button>{chips}</div><div id="groups">{''.join(cards)}</div></main><script>const q=document.querySelector('#q');let cat='ALL';function n(s){{return String(s||'').normalize('NFKC').toLowerCase().replace(/[\\s・･._\\-ー()（）]/g,'')}}function run(){{const z=n(q.value);document.querySelectorAll('.term').forEach(a=>{{const okCat=cat==='ALL'||a.dataset.cat===cat;const okQ=!z||n(a.textContent).includes(z);a.style.display=okCat&&okQ?'block':'none'}});document.querySelectorAll('.group').forEach(g=>{{g.style.display=[...g.querySelectorAll('.term')].some(a=>a.style.display!=='none')?'block':'none'}})}}q.oninput=run;document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{{cat=b.dataset.filter;run()}})</script></body></html>'''
...
def write_term_pages(terms):
    for t in terms:
        ...
        source=''
        if t.get('source_url'):
            source=f'<p><a class="back" href="{html.escape(t["source_url"])}" target="_blank" rel="noopener">出典：{html.escape(t.get("source_name") or "参照元")}</a></p>'
        page=f'''<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{html.escape(t['term'])}とは｜JRA AI 競馬ワード</title>{base_css()}</head><body><main class="wrap"><header class="top"><a href="../../app/">JRA AI</a><a href="../">ワード索引</a></header><section class="hero"><div class="reading">{html.escape(t.get('reading') or '')}</div><h1>{html.escape(t['term'])}</h1><p>{html.escape(category(t))}</p></section><section class="term"><p class="summary">{html.escape(t.get('summary') or t.get('short_definition') or 'サイト内で使われている用語です。')}</p>{f'<div class="aliases">関連語：{html.escape("・".join(aliases))}</div>' if aliases else ''}{source}</section><a class="back" href="../">← ワード索引へ戻る</a><br><a class="back" href="../../consult/?word={urllib.parse.quote(t['term'])}">AIにこの言葉を聞く</a></main></body></html>'''
        (d/'index.html').write_text(page,encoding='utf-8')

def main():
    terms=load_terms()
    write_index(terms);write_term_pages(terms)
