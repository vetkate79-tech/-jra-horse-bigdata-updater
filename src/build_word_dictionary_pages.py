...
ROOT=Path('.')
DOCS=ROOT/'docs'
# Public dictionary only uses definitions that are complete enough to stand alone.
# Auto-detected candidates remain an internal review queue until curated.
...
def write_index(terms):
    ...
    page=f'''<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>競馬ワード索引｜JRA AI</title>{base_css()}</head><body><main class="wrap"><header class="top"><a href="../app/">JRA AI</a><a href="../consult/">相談</a></header><section class="hero"><h1>競馬ワード索引</h1><p>サイトで使っている言葉をカテゴリから探せます。各ページだけで意味が分かるように説明しています。</p></section><input id="q" class="search" type="search" placeholder="ワード・読み方・関連語を検索"><div class="cats"><button class="chip" data-filter="ALL">すべて {len(terms)}</button>{chips}</div><div id="groups">{''.join(cards)}</div></main><script>const q=document.querySelector('#q');let cat='ALL';function n(s){{return String(s||'').normalize('NFKC').toLowerCase().replace(/[\\s・･._\\-ー()（）]/g,'')}}function run(){{const z=n(q.value);document.querySelectorAll('.term').forEach(a=>{{const okCat=cat==='ALL'||a.dataset.cat===cat;const okQ=!z||n(a.textContent).includes(z);a.style.display=okCat&&okQ?'block':'none'}});document.querySelectorAll('.group').forEach(g=>{{g.style.display=[...g.querySelectorAll('.term')].some(a=>a.style.display!=='none')?'block':'none'}})}}q.oninput=run;document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{{cat=b.dataset.filter;run()}})</script></body></html>'''
...
def write_term_pages(terms):
    for t in terms:
        ...
        page=f'''<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{html.escape(t['term'])}とは｜JRA AI 競馬ワード</title>{base_css()}</head><body><main class="wrap"><header class="top"><a href="../../app/">JRA AI</a><a href="../">ワード索引</a></header><section class="hero"><div class="reading">{html.escape(t.get('reading') or '')}</div><h1>{html.escape(t['term'])}</h1><p>{html.escape(category(t))}</p></section><section class="term"><p class="summary">{html.escape(t['summary'])}</p>{f'<div class="aliases">関連語：{html.escape("・".join(aliases))}</div>' if aliases else ''}{source}</section><a class="back" href="../">← ワード索引へ戻る</a></main></body></html>'''
