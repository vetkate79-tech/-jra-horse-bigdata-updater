...
def main():
 ...
 report={
  ...
  'active_workflow_count':len(active),'required_workflow_count':len(REQUIRED),
  'active_workflows':sorted(active),'required_workflows':sorted(REQUIRED),
  'missing_required':missing,'unexpected_active_workflows':unexpected,
  'checks':checks,'blockers':blockers,
  'independent_subsystems':cfg.get('independent_subsystems'),
  'production_flow':cfg.get('production_flow'),
  'note':'PASS means orchestration shape and subsystem boundaries are clean. It does not claim model predictive accuracy.'
 }
