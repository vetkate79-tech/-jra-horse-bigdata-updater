...
function weeklyRowsForDisplay(rows){
  ...
  else {sat=addDays(-2);sun=addDays(-1)} // Monday keeps the just-finished weekend visible.
  const dates=new Set([sat,sun]);
  const selected=rows.filter(x=>dates.has(x.date)&&x.scored);
  const label=selected.length?`${sat}〜${sun} / ${selected.length}レース結果接続`:'今週の結果接続待ち';
  return {waiting:false,rows:selected,label};
}
